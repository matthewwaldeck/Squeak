...
$logPath = "C:\Temp\Squeak\'Maintenance_$env:COMPUTERNAME'_$(get-date -f yyyy-MM-dd-HHmm).log" #Log file destination

function dirty_work {
  #Ensures creation of maintenance folder
  $path = Test-Path -Path "C:\Temp\Squeak\"
  if ($path -eq $false) {
    New-Item -Path "C:\Temp\Squeak\" -ItemType Directory | Out-Null
  ...
  "Log file will be written to C:\Temp\Squeak"
}
